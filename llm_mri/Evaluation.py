...
class Evaluation:
    ...
    def evaluate_model(self, n_splits:int = 5, test_size:float = 0.2, random_state:int = 42):
        ...
        results["reduced_classification_report"] = classification_report(yr_test, yr_pred, output_dict=True)
        results["full_classification_report"] = classification_report(yf_test, yf_pred, output_dict=True)

        #cross-validation with StratifiedKFold
        skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=random_state)

        results["reduced_cv_scores"] = cross_val_score(clf_reduced, reduced_hidden_states, y_reduced, cv=skf, scoring="accuracy")
        results["full_cv_scores"] = cross_val_score(clf_full, full_embeddings, y_embeddings, cv=skf, scoring="accuracy")
